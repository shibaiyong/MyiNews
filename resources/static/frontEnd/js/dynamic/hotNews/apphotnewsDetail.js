...
function getDetail(){
	...
	$.ajax({
        url : context+'/wechat/apphotnewsdetail',//这个就是请求地址对应sAjaxSource
        data:dataParam,
        type : 'get',
        dataType : 'json',
        async : true,
...
function fnweixin(wxtitle,wxdesc){
	...
	if(url.indexOf('?')>-1){
		...
		var ajaxUrl = "http://"+txtUrl+"/data/wechat/gotoapphotnewsdetail/"+$('.webpageCode').val()+"?"+addUrl;
	}else{
		var ajaxUrl = "http://"+txtUrl+"/data/wechat/gotoapphotnewsdetail/"+$('.webpageCode').val();
	}
