...
//	$('#example-navbar-collapse').find('ul.navbar-nav').find('li:eq(6)').addClass('active');
    ...
    //获取最新报道
    fetchlatestNews('.latestReportContainer',ctx + '/eventCustom/front/getEventNewsLatest');
    //获取首发报道
    fetchfirstNews('.firstReportContainer',ctx + '/eventCustom/front/getEventNewsFirst');
    ...
    table = $('.hotEventPredictConTable').DataTable({
        ...
        sAjaxSource : ctx+'/eventCustom/front/pageEventNews',//服务器请求
...
function eventTimeLine(eventCode){
    ...
    $.ajax({
        ...
        async: true,
        // 同步执行
        url: ctx + "/eventCustom/front/timeaxis/"+eventCode,
        dataType: "json",
        // 返回数据形式为json
...
function reportCountMediaData(eventCode){
    $.ajax({
        type : "get",
        async : true, //同步执行
        url : ctx+"/eventCustom/front/reportCountMedia",
        data: {
            eventCode : eventCode
        },
        dataType : "json", //返回数据形式为json
