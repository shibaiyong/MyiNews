...
$(function(){
	...
	$('#example-navbar-collapse').find('ul.navbar-nav').find('li:eq(3)').addClass('active');

    /*alert($('.linkLastNewsDataTable').width());*/
    $('.stuckMenu.isStuck').css({
      'width':$('.linkLastNewsDataTable').width(),
    });
      ...
      /*'sPaginationType': "bootstrap",*/
        ...
        "rowCallback" : function(row, data, index) {
        	...
        	if(null != data.similarNum && ""!=data.similarNum){
        		var similarNews = '<a href="similarNewsList/'+data.webpageCode+'" target="_blank" >('+data.similarNum+')</a>';
        	...
        	if(null != data.relevantNum && ""!=data.relevantNum){
          		var relevantNews = '<a href="relevantNewsList/'+data.webpageCode+'" target="_blank">('+data.relevantNum+')</a>';
    ...
    table2 = $('.dataConThumbnailTable').DataTable({
      ...
      'sPaginationType': "bootstrap",
        ...
        "rowCallback" : function(row, data, index) {
        	...
        	var content = '<div class="media"><a class="media-left" href="javascript:loadTopNews('+data.innerid+')"> <img src="'+data.picPath+'" alt="'+data.title+'" class="img-thumbnail" /></a>'
        				...
        				+ '<li class="list-group-item"><a href="'+relevantNews+'">'+relevantNum+' [相关新闻]</a></li></ul>'
...
function defaultChoose(){
    ...
    if('' == clueId || null == clueId || undefined == clueId){
    	...
    	$.ajax({
	        url : ctx+'/custom/front/clue/findClueById/'+clueId,
	        type : 'get',
	        dataType : 'json',
	        async : true,
	        success : function(data) {
