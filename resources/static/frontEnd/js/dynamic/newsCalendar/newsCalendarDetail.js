...
$(function(){
	...
	$('.sharingBtn').click(function(){
		...
		qrcode.makeCode('http://'+txtUrl+$('#context').val()+'/data/wechat/gotoappnewsdetail/'+$('.webpageCode').val()+'/0');
	})
...
function getNewsDetailVal(){
	...
	$.ajax({
        url: ctx + '/latest/front/getHistoryNewsDetail', //这个就是请求地址对应sAjaxSource
        data:dataParam,
        type : 'get',
        dataType : 'json',
        async : true,
...
//            	发布时间、来源、爬取来源等
            	...
            	if(obj.releaseDatetime != null && obj.releaseDatetime != ''){
								//解决Safari兼容问题
								// var time = obj.releaseDatetime.substring(0,19);
								// releaseDatetime =time.replace(/T/g, " ");
								releaseDatetime = new Date(obj.releaseDatetime).formatDate('yyyy-MM-dd hh:mm:ss');;
            	}else{
            		releaseDatetime = ' - '
            	}
...
function showCommentTable(webpageCode){
	...
	$('.commentHotConTable1').DataTable({
        ...
        "rowCallback" : function(row, data, index) {
        	...
        	var releaseDatetime;
            //解决Safari兼容问题
            var time = data.releaseDatetime.substring(6,16);
            releaseDatetime =time.replace(/T/g, " ");
