...
function searchFamiliar() {
    $('.searchWechat').click(function () {
        ...
        } else if (wechatNum.indexOf(' ') != -1) {
            $('.numTip').html('<i class="fa fa-exclamation-circle red"></i> 微信号不合法，请重新输入!');
        }else {
            $('.loading-zhe').show();
            $.ajax({
...
function conserveWechat() {
    ...
    if (wechatName == '') {
        ...
        $.ajax({
            ...
            data: {
                wechatID: wechatID,
                wechatName: wechatName.replace(/\s/g, ""),
                biz: biz
            },
            type: 'get',
            dataType: 'json',
            ...
            success: function (data) {
                ...
                if (data.result) {
                    ...
                    var content = $('.submenu').eq(3)[0].innerHTML
                    content += "<li class='mdynews' data-innerid=" + biz + ">" + wechatName + "</li>"
            ...
            if (data.result) {
                ...
                if (obj.length > 0) {
                    ...
                    if (content != '') {
                        ...
                        $('.link').eq(3).find("i")[0].style.visibility = "hidden";
                    }
                    setTimeout(function () {
                        $('.submenu').eq(3)[0].innerHTML = content;
                        $('.submenu').eq(3).find('li').click(function () {
