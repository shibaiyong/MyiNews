...
//        			{"name":"sourcesLevelTwo","value":sourcesLevelTwo},
        ...
        "rowCallback": function (row, data, index) {
            ...
            content = '<div class="dropdown">'
                ...
                + '</li>'
                + '<li role="presentation" >'
                + '<a role="menuitem" href="javascript:deleteNews(\'' + data.webpageCode + '\',\'' + data.releaseDatetime + '\')">删除</a>'
                + '</li>'
                + '</ul>'
                + '</div>'
            ...
            $('td:eq(0)', row).html('<span class="check-box check-child " data-webpagecode="' + data.webpageCode + '"  data-time="' + data.releaseDatetime + '"><i class="fa fa-check"></i></span>')
        },
        columns: [//显示的列
            {"bSortable": false},
            {
        ...
        $('.latestNewsConTable').itemCheck({   //给每一条新闻增加单击的事件
            'itemFun': function ($this, statusItem) {
                if (statusItem) {
                    ...
                    timeChecked.push($this.attr('data-time'))
                } else {
...
function deleteNews(id, time) {
    var params = {}
    if (id && time) {
        params.id = id;
        params.time = time
    } else {
        if (!webpagecodeChecked || webpagecodeChecked.length == 0) {
            return
        }
        params.id = webpagecodeChecked
        params.time = timeChecked
    }
    $.ajax({
        url: ctx + "/deleteWebPageNews",//这个就是请求地址对应sAjaxSource
        type: 'get',
        data: params,
        dataType: 'json',
        success: function (data) {
